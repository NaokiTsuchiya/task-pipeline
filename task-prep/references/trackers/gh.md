...
- **gate 宣言時の sha 記録**: `gate-light` を付けるとき (新規作成・深掘りの本文更新のどちらでも) は、本文に専用の 1 行 `裏取り時点: <sha>` を追加する (`<sha>` は `git -C <target project> rev-parse HEAD` の値)。書式は `依存:` / `未確定:` と同じ「本文の専用行を機械的に走査する」プレーンテキスト行の慣習に従う (末尾への追記を推奨するが必須ではない)。gate 宣言そのものは変更しない — 引き続き `gate-light` ラベルのみで表現し、この sha 行はラベルでは表現できない値を本文に置くためのものである。**既存の (sha未記録の) gate:light issue に遡って追記しない** — 今後新規に `gate-light` を付与する issue にだけ適用する。
