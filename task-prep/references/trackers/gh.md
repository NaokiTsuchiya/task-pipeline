...
- **昇格スキャン**: `list_issues (state: OPEN, labels: ["pending-deps"])` → 各 issue の本文から `依存:` を読み、全依存が解決済み (上記の判定) かつ本文に `未確定:` が無いものだけ、ラベルを `pending-deps` から `ready` に入れ替える (全置換規則で。`gate-light` と `priority-*` が付いていれば残す)。**候補に出るのはこの瞬間からなので、優先度の段はここで判断する** (SKILL.md「優先度」)。
