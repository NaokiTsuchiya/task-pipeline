...
- **合議の不変条件 (`audit_mode` が `dual` の検証側)** — `dual` の検証は**異種モデル 2 体の合議**であり、その 2 体の指定はこの 2 箇所だけで受ける: 段 1 の `verify_provider=<spec>,<spec>` (カンマ区切り) と、段 2 の `providers_by_class.high.audit` の **JSON 配列** (`["claude/...", "omp/..."]`。class が `high` でなくても `dual` に昇格したタスクは同じ行を引く — 検証側の指定を 1 箇所に閉じるため)。満たすべきは 4 つで、**1 つでも欠ければその検証ゲートは成立しない**: (a) spec がちょうど 2 件、(b) provider が互いに違う、(c) モデルファミリー (系統) が互いに違う、(d) 双方の系統が値から確かめられる (`omp` のように複数系統を載せる provider では model を明示する)。**満たせないときに単一の verifier へ落とすことはしない** — 誤 PASS は沈黙するので、弱い合議は「合議が無い」より危険である。扱いは検証エージェントを起こせないときと同じ終端 (経路節 項 6 の `block` + 通知) で、手順は `playbooks/dual-verifier.md`「不変条件違反」にある。**判定は `scripts/dual-verifier.ts` が行い、違反の種類 (`reason`) をそのまま返す。`audit_mode` の導出 (`risk: high` の床は `dual`) は `scripts/task-policy.ts` が正である。**
- **配列は合議専用の形である** — `providers_by_class.high.audit` 以外の場所 (`providers.audit`、`impl` の各段) に配列を書いても**単一の spec としては採らず、その段を素通りして次の段へ落ちる**。単一解決が配列の片方を黙って選ぶと、合議が沈黙のうちに単一検証へ降格するからである。`dual` の検証側で 3 件以上を書いた場合も、黙って 2 件に削らず違反として扱う。
...
- **起動パラメータ** — `--title "task-pipeline <役割> <タスク id>"` (`paseo ls` で役割とタスクが読めるようにする)、`--label task-pipeline=<役割>` と `--label task-pipeline-task=<タスク id>` の 2 本 (`--label` は繰り返せる。役割別のコスト回収と、下記の重複確認に効く)、`--cwd <そのタスク専用 worktree の絶対パス>`、**`--workspace <workspaceId>`** (下記「所有 workspace の記録と安全な後始末」節で `paseo workspace create --isolation local --path <同じ worktree> --json` により明示生成した Task Cell の id。executor・verifier どちらの起動にも常に付ける。呼び出し元が top-level か agent-scoped かによらず `--cwd` を確実に効かせるためで、workspace を渡さない agent-scoped の呼び出しでは `--cwd` が無視され呼び出し元自身の workspace を継承することが #145 の実機検証で確定している — workspace を渡さないと `paseo inspect --json` の `Cwd` が呼び出し元の workspace のままだったのに対し、そのタスク専用の workspace を渡すと `Cwd` が意図した worktree と一致した: https://github.com/NaokiTsuchiya/task-pipeline/issues/145#issuecomment-5369845395)。**`executor` には `--output-schema` を付けない** (経路節 項 2)。**agentId は `paseo run -d --json` の stdout から取るが、先頭に workspace 作成の通知行が混じるので「最初の `{` から後ろ」を JSON として読む。**
...
Paseo 経路で起こす executor/verifier は、**タスク専用の workspace (Task Cell) を明示生成してから**その workspace で起動する (gh-157)。生成は `paseo workspace create --isolation local --path <そのタスク専用 worktree の絶対パス> --title "task-pipeline <役割> <タスク id>" --json` で、応答 (stdout の JSON) の `workspaceId` がその exact な id である。起動側はその id を `paseo run -d --workspace <workspaceId> --json` に渡す (この形では新しい workspace は作られず、stderr に `Using workspace <id>` の通知が出るだけ。cwd は workspace 生成時の `--path` が使われる)。**呼び出し元が top-level か agent-scoped かによらず、毎回そのタスクだけの owned workspace になる。**

- **所有の判定** — 上のとおり `paseo workspace create` の応答から確定する。**通知行 (`Created workspace ...`) のパースには依存しない** — 実測 (paseo 0.4.0) でこの行は stdout ではなく stderr に出ており、出力先の版差で所有を取りこぼすと後から確定できない。`paseo inspect --json` の応答には `workspaceId` フィールドが無い。
...
- **記録の書き手** — Paseo 経路の起動を行う側が書く。Driver 経路では `scripts/pipeline-driver.ts` の takeover が **workspace の生成直後 (エージェント起動より前) に 1 回**、起動成功後に `agent_id` を埋めてもう 1 回書く (起動に失敗しても記録が残るので、生成した workspace が畳めなくなることがない)。
- **後始末は Cleanup Outbox とスイープが行う (人は畳まない)** — `state.ts retire` / `state.ts withdraw` が台帳の `cleanup_outbox` に後始末の意思を**同じ書き込みで**積み、常駐 Driver が低頻度のスイープでこの記録を読んで `paseo workspace archive <workspace_id>` を呼び、`archived_at` を埋めてから `state.ts cleanup-resolve --id <id>` で意思を落とす (`docs/state-cli-contract.md` の `cleanup-resolve` 節)。**手動 CLI で `retire`/`withdraw` した分も同じ経路に乗る。** Driver が動いていない間は意思が台帳に残り、次に起きたときのスイープで回収される。archive が失敗し続けたエントリは `attempts` が上限に達した時点で残置され、`state.ts get` から人が気づける。
