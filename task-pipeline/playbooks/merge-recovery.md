...
**owned workspace の後始末は自分でやらない** (gh-157): `state.ts retire` が `.task-pipeline/runs/<id>/paseo-workspace.json` に記録された owned workspace の archive を意思 (Cleanup Intent) として台帳に積み、常駐 Driver の冪等スイープが `paseo workspace archive` を呼んで `archived_at` を埋める (`playbooks/agent-launch.md` の「所有 workspace の記録と安全な後始末」節)。ここで手で archive を呼ぶ必要はなく、**Driver が動いていない間も意思は台帳に残るので取りこぼさない。**
