...
3. 実行エージェントを **background で 1 体** 起動する (**provider・model・mode と経路と起動パラメータは起動の直前に `playbooks/agent-launch.md` で決める** — 下のプロンプト文面は変えない)。プロンプトはこの 5 行のみ:
   ```
...
4. **以降、このタスクの進行は実行エージェントの停止の検知が駆動する。検知の仕方も、実行エージェントへの送信手段も、手順 3 で選んだ経路で決まる** (`playbooks/agent-launch.md`)。**どちらの経路でも、稼働中の実行エージェントに作業指示を送ってはならない。**
   ...
   - **Paseo 経路** → **通知は来ない** (CLI に受け口が無い。実測と選択の根拠は `docs/paseo-notify-on-finish-2026-08.md` の推奨)。代わりに**次のイテレーションでポーリングして読みに行く** — 受け皿は `playbooks/inflight.md` の **`wait` (`reason: executor-alive`)** で、そこで status と protocol 行を読む (読み方と、消費済みの行を再検知しないための鮮度規則は `playbooks/agent-launch.md`)。送信手段は `paseo send --no-wait`。dynamic 配下では同じく ScheduleWakeup (1800 秒) を予約するが、こちらはフォールバックではなく**本線の駆動**であり、**フェーズ間の遅延はイテレーション間隔まで伸びる**。
