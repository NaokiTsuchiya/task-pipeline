...
2. `issue_read(method: "get")` で現在の labels と assignees を読む。ラベルは名前の配列で返ることもオブジェクトの配列で返ることもあるので、名前だけを取り出して使う。**この名前の集合に `gate-light` があるかどうかを覚えておく** — 下記「タスク本文の書き出し」の frontmatter と応答に使う。
