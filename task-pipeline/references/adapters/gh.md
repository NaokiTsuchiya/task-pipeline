...
パイプラインが**書く**ラベルは **`in-review` と `blocked` の 2 つだけ** (名前は `review_label` / `blocked_label` で変更できる)。以下ではこの 2 つを **状態ラベル**と呼ぶ。

これとは別に **読むだけのラベルが 4 つ**ある: `gate-light` (gate 宣言。task-prep/SKILL.md「gate 宣言」)、`risk-high` (risk 宣言。同「risk-high 宣言」) と `priority-high` / `priority-low` (承認候補の優先順位。下記 `list` 手順 8)。**パイプラインはこの 4 つを付け外ししない。** 状態ラベルではないので候補判定にも使わず、`list` のクエリにも入れない。名前は固定である (状態ラベルと違い書き込まないので、チームのラベル運用と衝突しても実害が無い)。
...
- **GitHub ネイティブの依存関係 (blocked by) は候補の判定に使わない。** ネイティブ依存で「blocked by」が張られている issue でも、上記の候補条件 (状態ラベルなし・assignee なし・PR 紐付けなし) を満たせば候補になる。GitHub MCP に依存関係を読む口が無く、検索修飾子も無いためで、`list` のクエリに落とす手段がそもそも無い。**依存で候補を絞るのは task-prep の仕事** (依存の正は issue 本文の `依存:` 行、ゲートは `ready` ラベル。task-prep/references/trackers/gh.md)。
...
6. 候補を issue 番号の昇順に並べる (応答を安定させるための並びであって、実行順ではない — 承認順は SKILL.md「承認」手順 2 の priority の段と段内トリアージが決める)。
...
8. 応答: `{"tasks": [{"id": "gh-<番号>", "title": "...", "priority": "high|low", "labels": ["bug", ...], "milestone": {"title": "...", "due_on": "..."}, "updated_at": "..."}]}` のみ。本文や取得した生データを応答に含めない。
   - `priority` は手順 3 で読み直したラベルから決める: `priority-high` があれば `"high"`、`priority-low` があれば `"low"`、**どちらも無ければキーごと省く** (= 既定の中位)。両方付いていたら `"high"` を採る (人の付け間違いで候補が沈むより浮く方が害が小さい)。この 2 つも `gate-light` / `risk-high` と同じく**読むだけのラベル**で、パイプラインは付け外ししない。
   - `labels` は**パイプラインが使うラベルを除いた残り**を名前の配列で返す (`in-review` / `blocked` / `ready` / `pending-deps` / `gate-light` / `risk-high` / `priority-high` / `priority-low` を除く。`ready` を除くのは `?label=ready` 起動では全件に付いていて情報量が無いため)。空になるならキーごと省く。オーケストレーターはこれを順位付けの材料として渡す (SKILL.md「承認」手順 2) ので、**プロジェクト側の語彙 (`bug` / `security` / 種別) だけが残るようにする**。
   - `milestone` は付いていれば `{"title": ..., "due_on": ...}` を返す (`due_on` が無ければ title だけ)。無ければキーごと省く。
   - `updated_at` は手順 3 の読み直しで得た issue の値をそのまま返す。オーケストレーターが順位キャッシュの再利用判定に使う — **本文が書き換わったのに前回の並びが再利用されるのを防ぐのが目的**なので、丸めたり省いたりしない。
...
2. `issue_read(method: "get")` で現在の labels と assignees を読む。ラベルは名前の配列で返ることもオブジェクトの配列で返ることもあるので、名前だけを取り出して使う。**この名前の集合に `gate-light` と `risk-high` があるかどうかを覚えておく** (2 つとも独立に見る) — 下記「タスク本文の書き出し」の frontmatter と応答に使う。**同じ応答の `created_at` と `updated_at` も覚えておく** — 下記の本文取得で、issue を 1 件に絞るキーと、取れた本文が最新かを照合するキーに使う (このために追加の呼び出しをする必要は無い)。
...
- **`risk: high` の行は、手順 2 で読んだラベルに `risk-high` があるときだけ入れる** (無ければ行ごと省く。`risk: low` とは書かない)。扱いは `gate: light` と同型で、本文中の `<!-- task-pipeline:risk=high -->` は判定に使われない (gh で正となるのはラベル)。**2 つは独立に転写する** — 両方のラベルが付いていれば両方の行を書く (宣言としては背反だが、それを判定して落とすのはアダプタの仕事ではない。読み手側の扱いは `playbooks/agent-launch.md`「タスクの class」)。
- **応答に `gate_declared: true|false` と `risk_declared: true|false` を含める** (`{"ok": true, "gate_declared": true, "risk_declared": false}`)。それぞれラベルの有無をそのまま返す。オーケストレーターが自分の grep 結果と突き合わせて、この書き出しが宣言を落としたことを検知できるようにするため。書き出しに失敗したときも、ラベルを読めているならその値を返す。
- **本文の取得に `issue_read` の `body` を第一候補として使わない** (使えるのは下記「三次」の最後の手段としてだけ)。GitHub MCP の `issue_read (method: "get")` は本文を HTML エスケープして返し (`>` → `&gt;`、`'` → `&#39;`、`"` → `&#34;`、コードブロックの中も含む)、**`<!-- ... -->` の行を丸ごと落とす** (実測。`../../docs/gate-declaration-2026-08.md` §2 / §5)。落ちるのは gate 宣言だけではない — 受け入れ条件に書かれた `<...>` 表記も同じ理由で消える。**`list_issues` の `body` も同じ欠落をする**ので代替にならない (同 §5)。本文は次の **3 段**で取り、**上の段が成立したらそこで止める**。

  **一次 — MCP の raw 経路 (既定)。** `search_issues` が返す `body` は**エスケープされず、`<!-- ... -->` 行も残る** (実測。同 §5)。issue 番号で直接引く修飾子は GitHub の検索構文に無いので、手順 2 で覚えた `created_at` で 1 件に絞る:
  ...
  返った `items` から **`number` が一致するもの**を選ぶ (同じ秒に作られた issue が複数あるので 1 件目を無条件に採らない)。**その `updated_at` が手順 2 の `issue_read` の `updated_at` と一致するときだけ** `body` を採用する。`search_issues` は検索インデックス越しなので直前の編集が反映されていないことがあり (このアダプタは `list` 手順 3 でも同じ遅延を扱っている)、**古い本文を掴んだまま気付かないことだけは避ける**ための照合である。次のいずれかなら一次は不成立で、二次へ落ちる: 0 件 / `number` の一致する item が無い / `updated_at` が食い違う / 呼び出しがエラー。

  **二次 — `gh` CLI の逐語経路。** 一次が不成立のときだけ使う。ここが「読み書きは GitHub MCP で行う」の唯一の例外である (状態の読み書き・検索・コメント取得は MCP のまま)。**実体バイナリを取り、時間上限を付けて実行する**:

  ```bash
  GH=$(which -a gh | grep '^/' | head -1)
  perl -e 'alarm shift; exec @ARGV' 30 "$GH" issue view <番号> --repo <owner/repo> --json body --jq .body
  ```

  - `which -a gh | grep '^/' | head -1` は `../executor.md` / `../pr-watcher.md` と同じ**実体バイナリ回避**。`gh` がパスワードマネージャのプラグイン等にエイリアスされていると、非対話セッションでは承認プロンプトを出せずに失敗する一方、実体のバイナリは認証済みで動くことがある。
  - **時間上限は `perl` の `alarm` で掛ける。** macOS には `timeout` / `gtimeout` が入っていない環境がある (実測。同 §5)。`perl` は標準で入っており、`alarm` + `exec` は打ち切りを**終了コード 142** (128+14 = SIGALRM) で返す — 実測確認済み。Bash ツール側の `timeout` パラメータ (35000 ms 程度) も併せて指定しておくと二重の保険になる。
  - **打ち切り (終了コード 142) は「`gh` が使えない」と同一視して三次へ進む。** 無人ループの中で本文取得が固まると、そのタスクは着手時点で止まる — 止まるより、欠落を明示した本文で進む方がよい。142 以外の非 0 終了、および出力が空のときも同じく三次へ。

  **三次 — エスケープされた本文で続行する。** `issue_read` が返した本文を使い、**その旨をタスクファイルの本文冒頭に 1 行書く** (executor / verifier が URL で原文に当たれるように)。一次・二次が成立したときはこの 1 行を書かない (本文が無傷なので印は要らない)。gate と risk はどちらもラベル経由なので、ここまで落ちても宣言は失われない。
...
- コメントは `comments` の件数が 1 以上のときだけ `issue_read(method: "get_comments")` で取得する。**`get_comments` に本文と同じエスケープは観測されていないが、山括弧を含むコメントでの実測はしていない** (同 §5)。コメントに `<...>` や `<!-- ... -->` が要るなら、本文側と同じ欠落を疑うこと。**コメントは古い順に返るので、件数から逆算して新しい方のページだけを取る**: `perPage: 10` として最終ページ (`ceil(件数 / 10)`) を取得し、必要な数に満たなければ 1 つ前のページも取る。
