...
これとは別に **`gate-light` ラベルを読む**。task-prep が付ける gate 宣言 (task-prep/SKILL.md「gate 宣言」) で、パイプラインは**読むだけで付け外ししない**。状態ラベルではないので候補判定にも使わず、`list` のクエリにも入れない。名前は固定である (状態ラベルと違い書き込まないので、チームのラベル運用と衝突しても実害が無い)。
