...
これとは別に **読むだけのラベルが 3 つ**ある: `gate-light` (gate 宣言。task-prep/SKILL.md「gate 宣言」) と `priority-high` / `priority-low` (承認候補の優先順位。下記 `list` 手順 8)。**パイプラインはこの 3 つを付け外ししない。** 状態ラベルではないので候補判定にも使わず、`list` のクエリにも入れない。名前は固定である (状態ラベルと違い書き込まないので、チームのラベル運用と衝突しても実害が無い)。
...
8. 応答: `{"tasks": [{"id": "gh-<番号>", "title": "...", "priority": "high|low", "labels": ["bug", ...], "milestone": {"title": "...", "due_on": "..."}, "updated_at": "..."}]}` のみ。本文や取得した生データを応答に含めない。
   - `priority` は手順 3 で読み直したラベルから決める: `priority-high` があれば `"high"`、`priority-low` があれば `"low"`、**どちらも無ければキーごと省く** (= 既定の中位)。両方付いていたら `"high"` を採る (人の付け間違いで候補が沈むより浮く方が害が小さい)。この 2 つも `gate-light` と同じく**読むだけのラベル**で、パイプラインは付け外ししない。
   - `labels` は**パイプラインが使うラベルを除いた残り**を名前の配列で返す (`in-review` / `blocked` / `ready` / `pending-deps` / `gate-light` / `priority-high` / `priority-low` を除く。`ready` を除くのは `?label=ready` 起動では全件に付いていて情報量が無いため)。空になるならキーごと省く。オーケストレーターはこれを順位付けの材料として渡す (SKILL.md「承認」手順 2) ので、**プロジェクト側の語彙 (`bug` / `security` / 種別) だけが残るようにする**。
...
2. `issue_read(method: "get")` で現在の labels と assignees を読む。ラベルは名前の配列で返ることもオブジェクトの配列で返ることもあるので、名前だけを取り出して使う。**この名前の集合に `gate-light` があるかどうかを覚えておく** — 下記「タスク本文の書き出し」の frontmatter と応答に使う。**同じ応答の `created_at` と `updated_at` も覚えておく** — 下記の本文取得で、issue を 1 件に絞るキーと、取れた本文が最新かを照合するキーに使う (このために追加の呼び出しをする必要は無い)。
...
- **本文の取得に `issue_read` の `body` を第一候補として使わない** (使えるのは下記「三次」の最後の手段としてだけ)。GitHub MCP の `issue_read (method: "get")` は本文を HTML エスケープして返し (`>` → `&gt;`、`'` → `&#39;`、`"` → `&#34;`、コードブロックの中も含む)、**`<!-- ... -->` の行を丸ごと落とす** (実測。`../../docs/gate-declaration-2026-08.md` §2 / §5)。落ちるのは gate 宣言だけではない — 受け入れ条件に書かれた `<...>` 表記も同じ理由で消える。**`list_issues` の `body` も同じ欠落をする**ので代替にならない (同 §5)。本文は次の **3 段**で取り、**上の段が成立したらそこで止める**。
  ...
  **三次 — エスケープされた本文で続行する。** `issue_read` が返した本文を使い、**その旨をタスクファイルの本文冒頭に 1 行書く** (executor / verifier が URL で原文に当たれるように)。一次・二次が成立したときはこの 1 行を書かない (本文が無傷なので印は要らない)。gate はラベル経由なので、ここまで落ちても宣言は失われない。
