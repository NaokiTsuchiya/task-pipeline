...
**やらなかったこと**: 計画の第 2 段 (`task-pipeline-watcher` を同じパターンで作る) は着手していない。
第 2 段は「まず verifier の実測結果を見てから」という条件付きで、その結果が「effort に価値なし」
だったため、watcher 側に effort を入れる根拠は無い。tools 制限だけを目的に 2 本目の
未検証のエージェント型を増やすのは、1 本目 (verifier) がまだ実環境で 1 度も走っていない現状では早い。
