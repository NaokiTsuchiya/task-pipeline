...
- マージされたら (リモートで検知するので、手元に取り込む前でも分かる) 通常どおり Done へ回収し、worktree を片付ける。未マージで PR を閉じた場合は、取り下げと見なして追従を止める (blocked にはしない)。取り下げたタスクはマージされないので回収も起きず、レビュー待ちのまま残る — そこで**次に候補を決めるときに「queue から外してよいか」を一度だけ聞かれる**。`approve=auto` では聞かずに 1 行報告するだけで、勝手に外しもせず done にはしない (要求が別経路で満たされたかどうかは、パイプラインには判定できないため)。
