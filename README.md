...
- **タスクはそれぞれ専用の git worktree (`.claude/worktrees/task-pipeline/<id>`、ブランチ `task-pipeline/<id>`) で実行される**ので、動いている間もあなたは同じリポジトリで普通に作業できる。worktree が消えるのはマージを回収して Done にしたときだけで、レビュー待ちや blocked では残る (未コミットの変更や途中成果物を失わないため)。
