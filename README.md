...
- **リポジトリルートに `TASK_PIPELINE_CHECKS.json` を置くと、gate 宣言のあるタスクの実装フェーズのゲートが LLM 検証からコマンド実行 (Shell-Check) に替わる** — 宣言された `checks` を実際に走らせ、終了コードと変更ファイルが承認済みスコープ内かだけで PASS/FAIL を決める (LLM 呼び出しゼロ)。マニフェストは**分岐元 (base) のスナップショットから読まれる**ので、実装エージェントが自分のブランチでチェックを差し替えることはできない。調査+計画と報告のゲートは LLM のまま — 散文の成果物はコマンドでは検証できないため。置いていないリポジトリでは何も変わらない。
...
- **待ち方は「定期的に見に行く」ではなく「変化したら起こされる」**。[scripts/watch-pr.sh](task-pipeline/scripts/watch-pr.sh) がバックグラウンドで PR の署名 (状態・head sha・CI ロールアップ・コメント数・レビュー数・スレッド総数・未解決スレッド数・コメントの最終更新時刻) を GraphQL 1 回ぶんずつ見張り、**変わるまでブロックして終了する**。CI だけでなく、**新しいレビューコメント・既存スレッドへの返信・コメントの編集**も検知対象 (いずれも実測確認済み)。**ただし下書き (レビューを送信する前の状態) は数えない** — 下書きは作成者本人の認証では API に返るので、ソロ開発では自分が書いている途中の指摘で追従が起きてしまう。送信された時点で検知される。**さらに見ている窓には上限がある** — PR 直下コメントとレビューは新しい側 50 件ずつ、1 スレッド内は新しい側 20 件まで、スレッド自体は直近に作られた/動きがあった側 100 本まで。これを外れるもの (窓より古い項目の本文編集、スレッド総数が 100 を超えるときの最も古い側のスレッドの resolve/unresolve) は検知されない。新規のコメント・レビュー・スレッドの投稿はいずれも totalCount で検知されるので、件数がいくつであっても取り落とさない。ポーリングするのはこのシェルであってモデルではないので、CI が終わった瞬間・コメントが付いた瞬間にちょうど 1 回だけ起きる。webhook の受け口を持てないローカルセッションで、反応の速さだけを webhook と同じにするための仕組み (`--watch` 系のコマンドと同じ発想)。
